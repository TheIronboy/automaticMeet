...
namespace automaticMeet
{
    public partial class automaticMeet : Form
    {
        ...
        public async void mainConnect(string email, string pass, string selectedCode, string messaggio, int speed)
        {
            ...
            IWebDriver chrome = new ChromeDriver(chromeService, options);
            ...
            {
                MessageBox.Show("Ho riscontrato un problema nella fase di login, assicurati di aver attivato l'impostazione (Consenti app meno sicure) (Non consigliato per gli account principali) e di aver disabilitato la verifica in due fattori nell'account google da cui stai facendo l'accesso. Usa questo link: https://myaccount.google.com/lesssecureapps");
                    ...
                    try
                    {
                        ...
                        while (quit == false)
                        {
                            try
                            {
                                ...
                                {
                                    quit = true;
                                    chrome.FindElement(By.XPath("/html/body/div[1]/c-wiz/div[1]/div/div[9]/div[3]/div[9]/div[2]/div[2]/div")).Click();

                                    button1.Enabled = true;
                                    progressBar1.Increment(1);

                                    MessageBox.Show("Riunione terminata.");
                                }
                                else
