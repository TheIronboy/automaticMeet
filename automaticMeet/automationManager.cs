...
namespace automaticMeet
{
    public partial class automationManager : Form
    {
        ...
        string[,] settingsFile = new string[10, 6];
        ...
        private void loadAutomationsSettings(string[] sessionData, string[,] settingsData, CheckBox[] enabledCheckBoxes, CheckBox[] addStringCheckBoxes, NumericUpDown[] numericUpDownsHours, NumericUpDown[] numericUpDownsMinutes, ComboBox[] comboBoxes, DomainUpDown[] stringToAdd, TextBox[] textBoxes)
        {
            using (StreamReader file = File.OpenText(@"C:\automaticMeet\" + sessionData[0] + @"\automationsSettings.txt"))
            {
        ...
        private void saveAutomationsSettings(string[] sessionData, string[,] settingsData, CheckBox[] enabledCheckBoxes, CheckBox[] addStringCheckBoxes, NumericUpDown[] numericUpDownsHours, NumericUpDown[] numericUpDownsMinutes, ComboBox[] comboBoxes, DomainUpDown[] stringToAdd, TextBox[] textBoxes)
        {
            for (int i = 0; i < settingsData.GetLength(0); i++)
            {
            ...
            using (StreamWriter file = File.CreateText(@"C:\automaticMeet\" + sessionData[0] + @"\automationsSettings.txt"))
            {
                for (int i = 0; i < settingsData.GetLength(0); i++)
                    for (int k = 0; k < settingsData.GetLength(1); k++)
        ...
        private void automationManager_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < settingsFile.GetLength(0); i++)
            {
                publicFunctionsRef.getCodesList(publicFunctionsRef.sessionFile, comboBoxes[i]);
            ...
            if (!File.Exists(@"C:\automaticMeet\" + publicFunctionsRef.sessionFile[0] + @"\automationsSettings.txt"))
                saveAutomationsSettings(publicFunctionsRef.sessionFile, settingsFile, enabledCheckBoxes, addStringCheckBoxes, numericUpDownsHours, numericUpDownsMinutes, comboBoxes, domainUpDowns, textBoxes);
            else
                loadAutomationsSettings(publicFunctionsRef.sessionFile, settingsFile, enabledCheckBoxes, addStringCheckBoxes, numericUpDownsHours, numericUpDownsMinutes, comboBoxes, domainUpDowns, textBoxes);
        }
        ...
        private async void button1_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < settingsFile.GetLength(0); i++)
            {
                if (Convert.ToBoolean(settingsFile[i, 0]))
                {
                    ...
                    while (true)
                    {
                        ...
                        if (DateTime.Now.Hour == Convert.ToInt32(hourAndMinutesExtractor(0, settingsFile[i, 2])) && DateTime.Now.Minute >= Convert.ToInt32(hourAndMinutesExtractor(1, settingsFile[i, 2])))
                        {
